...
Wykonywać operacje na issue przez API w sposób powtarzalny i audytowalny, gdy MCP GitHub albo `gh` nie zapewniają wymaganej operacji.

## Wymagania wejściowe

1. Hosting repozytorium: GitHub.
2. Owner i nazwa repozytorium.
3. Token API z właściwymi uprawnieniami.
4. Narzędzia `curl`, `jq` i `bash`.
...
5. Wykonaj operację przez `scripts/github-issues.sh` albo bezpośredni endpoint API.
